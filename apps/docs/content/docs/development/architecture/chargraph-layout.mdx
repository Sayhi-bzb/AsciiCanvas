...
the sender, including nested intervals. Its `sequence.activation` style role
...
outer corridor. Cycles and parallel edges remain ordinary relationships.
...
single-path labels remain route-placed. When continuous ELK attachments collapse
...
border cells. A bundle is one endpoint owner. State, Class, and ER retain their
existing endpoint policies.
...
routes before labels are placed and the scene is composed. Flow and State
additionally opt into readable routing: overlapping opposite endpoints share an
axis when possible. Route repair compares strict and relaxed orthogonal
