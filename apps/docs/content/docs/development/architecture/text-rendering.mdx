...
IR. CharGraph's built-in extension manifest registers Alert, Diff, JSON/YAML trees, Mermaid, and
math once for rendering and detection. The document feature registry binds those engine features to
...
muted structure role. Inline and block rules have independent switches and share one Math palette
