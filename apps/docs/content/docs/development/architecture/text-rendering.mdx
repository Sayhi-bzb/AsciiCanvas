...
`clipboard source -> document text-rendering worker -> styled protocol row spans -> Canvas command`
...
at or above 50,000 UTF-16 code units render in a module worker. The protocol layout is a streaming
grapheme pass; a 10,000-line result stays as row spans until Canvas applies one transaction.
