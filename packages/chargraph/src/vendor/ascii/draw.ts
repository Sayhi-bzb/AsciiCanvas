...
import { mkCanvas, copyCanvas, getCanvasSize, mergeCanvases, drawText, mkRoleCanvas, setRole, mergeRoleCanvases } from './canvas.js'
import type { RoleCanvas, CharRole } from './types.js'
...
export function drawArrow(
  ...
  const [pathCanvas, linesDrawn, lineDirs] = drawPath(graph, edge)
...
function drawPath(
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    const segment = drawLine(canvas, prevDC, nextDC, 1, -1, graph.config.useAscii, edge.style)
