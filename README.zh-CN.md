...
CharDesk 的输出也可以在编辑器之外使用。文本交换格式见 [`@chardesk/protocol`](packages/protocol/README.md)，框架无关的网页渲染见 [`@chardesk/viewer`](packages/viewer/README.md)，兼容字形资源见 [`@chardesk/fonts`](packages/fonts/README.md)。[CharGraph](https://chardesk.com/chargraph/) 可以将结构化源码呈现为可移植的 Unicode 文本。安装方式与 API 由各自的包文档负责。
